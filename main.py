...
def listar_produtos():
    produtos = products
    descricao_parcial = request.args.get('description_part')
    if descricao_parcial:
        descricao_parcial = descricao_parcial.lower()
        produtos = [p for p in produtos if descricao_parcial in p['product_description'].lower()]
    if request.args.get('preco_asc') == 'true':
        produtos = sorted(produtos, key=lambda p: p['product_price'])
    elif request.args.get('preco_desc') == 'true':
        produtos = sorted(produtos, key=lambda p: p['product_price'], reverse=True)
...
def obter_produto(product_id):
    produto = next(p for p in products if p['id'] == product_id)
